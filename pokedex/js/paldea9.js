const paldea = async (start = 899) => {

    clear()
    removeModal()
    const promises = [];
    
    for (let index = start; index < start + 20 && index < 1011; index++) {
    ...
    pokemonDataArray.forEach(async (data, index) => {
        ...
        card.addEventListener('click', async () => {
            ...
            const pokemonName = data.dataPokemon.name;
            try {
                const data = await getEvolutionData(pokemonName);
            } catch (error) {
                console.error(error);
            }
